...
class GeneratorService:
    ...
    def __init__(self, model_path: Optional[Path] = None, device: Optional[str] = None):
        ...
        Args:
            model_path: Path to the model checkpoint.
                Defaults to models/generator_model.pth
    ...
    def load_model(self) -> None:
        ...
        logger.info(
            f"Loading generator model from {self.model_path} on device {self.device}..."
        )
        ...
        try:
            ...
            logger.info(
                f"Model has num_grades={num_grades} (should be 19 for new models)"
            )

            if num_grades != 19:
                logger.warning(
                    f"Model has {num_grades} grades instead of "
                    "expected 19. This may be an old model trained "
                    "before the grade mapping fix. "
    ...
    def generate_problem(
        self, grade: str, temperature: float = 1.0, max_attempts: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        Raises:
            ...
            ValueError: If grade is invalid or no valid problem
                generated
        """
        ...
        try:
            ...
            logger.info(f"Generating problem at grade {grade} (label {grade_label})")
            ...
            logger.info(
                f"Successfully generated problem with "
                f"{len(formatted['moves'])} moves at grade {grade}"
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.is_loaded and self._generator is not None:
            ...
            if (
                info["min_grade_index"] is not None
                and info["max_grade_index"] is not None
            ):
                info["filtered"] = True
            else:
                info["filtered"] = False
