...
class TestCreateGridTensor:
    ...
    def test_example_json_problem(self):
        """Test with moves from example.json"""
        moves = [
        ...
        # Check start hold (J4)
        assert tensor[0, 3, 9] == 1.0
        assert np.sum(tensor[0]) == 1.0
        
        # Check middle holds (7 total)
        assert np.sum(tensor[1]) == 7.0
        
        # Check end hold (F18)
        assert tensor[2, 17, 5] == 1.0
        assert np.sum(tensor[2]) == 1.0
    ...
    def test_multiple_start_holds(self):
        """Test with multiple start holds"""
        moves = [
            {"description": "A1", "isStart": True, "isEnd": False},
            {"description": "B1", "isStart": True, "isEnd": False},
            {"description": "K18", "isStart": False, "isEnd": True}
        ]
        tensor = create_grid_tensor(moves)
        
        assert tensor.shape == (3, ROWS, COLS)
        assert np.sum(tensor[0]) == 2.0  # Two start holds
        assert tensor[0, 0, 0] == 1.0  # A1
        assert tensor[0, 0, 1] == 1.0  # B1
    
    def test_multiple_end_holds(self):
        """Test with multiple end holds"""
        moves = [
            {"description": "A1", "isStart": True, "isEnd": False},
            {"description": "J18", "isStart": False, "isEnd": True},
            {"description": "K18", "isStart": False, "isEnd": True}
        ]
        tensor = create_grid_tensor(moves)
        
        assert tensor.shape == (3, ROWS, COLS)
        assert np.sum(tensor[2]) == 2.0  # Two end holds
        assert tensor[2, 17, 9] == 1.0  # J18
        assert tensor[2, 17, 10] == 1.0  # K18
    
    def test_duplicate_position_same_type(self):
        """Test duplicate positions in same category (should just overwrite with 1.0)"""
        moves = [
            {"description": "F7", "isStart": False, "isEnd": False},
            {"description": "F7", "isStart": False, "isEnd": False}
        ]
        ...
        # Should still just be 1.0 at that position
        assert tensor[1, 6, 5] == 1.0
        assert np.sum(tensor[1]) == 1.0
    
    def test_case_insensitive_positions(self):
        """Test that lowercase positions work"""
        moves = [
            {"description": "f7", "isStart": True, "isEnd": False}
        ]
        tensor = create_grid_tensor(moves)
    ...
    def test_whitespace_in_positions(self):
        """Test that positions with whitespace work"""
        moves = [
            {"description": "  F7  ", "isStart": True, "isEnd": False}
        ]
        tensor = create_grid_tensor(moves)
    ...
    def test_missing_isStart_field(self):
        """Test that missing isStart raises error"""
        with pytest.raises(ValueError, match="missing 'isStart' field"):
            create_grid_tensor([
                {"description": "F7", "isEnd": False}
            ])
    
    def test_missing_isEnd_field(self):
        """Test that missing isEnd raises error"""
        with pytest.raises(ValueError, match="missing 'isEnd' field"):
            create_grid_tensor([
                {"description": "F7", "isStart": True}
            ])
    
    def test_invalid_isStart_type(self):
        """Test that non-boolean isStart raises error"""
        with pytest.raises(ValueError, match="'isStart' must be boolean"):
            create_grid_tensor([
                {"description": "F7", "isStart": "true", "isEnd": False}
            ])
    
    def test_invalid_isEnd_type(self):
        """Test that non-boolean isEnd raises error"""
        with pytest.raises(ValueError, match="'isEnd' must be boolean"):
            create_grid_tensor([
                {"description": "F7", "isStart": True, "isEnd": 1}
            ])
    ...
    def test_invalid_position_out_of_bounds_row(self):
        """Test that out-of-bounds row raises error"""
        with pytest.raises(ValueError, match="Invalid position"):
            create_grid_tensor([
                {"description": "F20", "isStart": True, "isEnd": False}
            ])
    
    def test_invalid_position_out_of_bounds_col(self):
        """Test that out-of-bounds column raises error"""
        with pytest.raises(ValueError, match="Invalid position"):
            create_grid_tensor([
                {"description": "Z7", "isStart": True, "isEnd": False}
            ])
...
class TestGetChannelCounts:
    ...
    def test_counts_return_type(self):
        """Test that counts are Python ints, not numpy types"""
        moves = [
            {"description": "F7", "isStart": True, "isEnd": False}
        ]
        tensor = create_grid_tensor(moves)
        ...
        assert isinstance(counts['start'], int)
        assert isinstance(counts['middle'], int)
        assert isinstance(counts['end'], int)
...
class TestTensorToMoves:
    ...
    def test_positions_are_sorted(self):
        """Test that returned positions are sorted"""
        moves = [
            {"description": "K18", "isStart": False, "isEnd": False},
            {"description": "A1", "isStart": False, "isEnd": False},
            {"description": "F7", "isStart": False, "isEnd": False}
        ]
        tensor = create_grid_tensor(moves)
        positions = tensor_to_moves(tensor)
        ...
        assert positions['middle'] == sorted(positions['middle'])
    
    def test_both_start_and_end(self):
        """Test conversion when hold is both start and end"""
        moves = [
            {"description": "F7", "isStart": True, "isEnd": True}
        ]
        tensor = create_grid_tensor(moves)
    ...
    def test_invalid_tensor_type(self):
        """Test that non-array raises error"""
        with pytest.raises(ValueError, match="tensor must be a numpy array"):
            tensor_to_moves([[1, 2, 3]])
    
    def test_invalid_tensor_shape(self):
        """Test that wrong shape raises error"""
        tensor = np.zeros((2, ROWS, COLS))
        with pytest.raises(ValueError, match="tensor must have shape"):
            tensor_to_moves(tensor)
...
class TestIntegration:
    ...
    def test_full_workflow(self):
        """Test complete workflow: moves -> tensor -> counts -> positions"""
        moves = [
            {"description": "J4", "isStart": True, "isEnd": False},
            {"description": "G6", "isStart": False, "isEnd": False},
            {"description": "F7", "isStart": False, "isEnd": False},
        ...
        counts = get_channel_counts(tensor)
        assert counts['start'] == 1
        assert counts['middle'] == 7
        assert counts['end'] == 1
        
        # Convert back to positions
        positions = tensor_to_moves(tensor)
        assert positions['start'] == ['J4']
        assert len(positions['middle']) == 7
        assert positions['end'] == ['F18']
        
        # Verify all middle positions
        expected_middle = {'A11', 'A16', 'B8', 'B9', 'D15', 'F7', 'G6'}
        assert set(positions['middle']) == expected_middle
    
    def test_all_corner_positions(self):
        """Test that all four corners can be represented"""
        moves = [
            {"description": "A1", "isStart": True, "isEnd": False},
            {"description": "K1", "isStart": False, "isEnd": False},
            {"description": "A18", "isStart": False, "isEnd": False},
            {"description": "K18", "isStart": False, "isEnd": True}
        ]
        ...
        assert 'A1' in positions['start']
        assert 'K1' in positions['middle']
        assert 'A18' in positions['middle']
        assert 'K18' in positions['end']
